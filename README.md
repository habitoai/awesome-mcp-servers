...
- [Melaya](https://melaya.org/en/product/mcp) - Remote MCP server that operates a paired Android phone through its accessibility tree, tapping, typing and swiping inside any app with no per-app integration or vendor API. Also drives a paired browser and runs agent pipelines. OAuth 2.1 with dynamic client registration, so there is no API key and nothing to install. Scoped to apps the user allow-lists, with on-device approval before anything posts under their identity. Endpoint: https://api.melaya.org/mcp
